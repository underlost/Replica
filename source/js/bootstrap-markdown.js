...
!function ($) {
  ...
  Markdown.prototype = {
    ...
    __buildButtons: function (buttonsArray, container) {
      ...
      for (i = 0; i < buttonsArray.length; i++) {
        ...
        for (y = 0; y < btnGroups.length; y++) {
          ...
          var z, buttons = btnGroups[y].data, btnGroupContainer = $('<div/>', { 'class': 'btn-group-vertical' });
          for (z = 0; z < buttons.length; z++) {
            var button = buttons[z], buttonToggle = '', buttonHandler = ns + '-' + button.name, btnText = button.btnText ? button.btnText : '', btnClass = button.btnClass ? button.btnClass : 'btn btn-default btn-sm', tabIndex = button.tabIndex ? button.tabIndex : '-1';
            if (button.toggle === true) {
              buttonToggle = ' data-toggle="button"';
            }
            // Attach the button object
            btnGroupContainer.append('<button class="' + btnClass + ' btn-small" title="' + button.title + '" tabindex="' + tabIndex + '" data-provider="' + ns + '" data-handler="' + buttonHandler + '"' + buttonToggle + '><i class="' + button.icon + '"></i> ' + btnText + '</button>');
    ...
    showEditor: function () {
      ...
      if (this.$editor == null) {
        ...
        if (options.savable) {
          ...
          editorFooter.append('<button class="btn btn-success" data-provider="' + ns + '" data-handler="' + saveHandler + '"><i class="fa fa-check-circle"></i> Save</button>');
    ...
    showPreview: function () {
      var options = this.$options, callbackContent = options.onPreview(this),
        // Try to get the content from callback
        container = this.$textarea, afterContainer = container.next(), replacementContainer = $('<div/>', {
          'class': 'md-preview',
