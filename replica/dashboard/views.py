...
from replica.pulse.forms import EntryModelForm, QuickEntryModelForm, DraftModelForm, TopicModelForm, EntryTypeModelForm, MediaModelForm
from replica.pulse.models import Entry, Draft, Topic, Media, EntryType
...
def EntryDelete(request, entry_guid):
	...
	if request.method == 'POST':
		entry.delete()
		return redirect('Replica:Index')
	return render(request, 'replica/dashboard/delete-confirm.html', {'object': entry, 'content_type': 'Entry'})
...
class ReplicaViewMixin(object):
	...
	def get_queryset(self):
		if self.request.user.is_staff:
			...
			return Entry.objects.posts().filter(user=self.request.user)
...
class EntriesForTopic(ListView):
	paginate_by = 20
	template_name = 'replica/dashboard/topic_entry_list.html'

	def get_queryset(self):
		self.topic = get_object_or_404(Topic, slug=self.kwargs.pop('topic_slug'))
		return Entry.objects.filter(topic=self.topic).filter(user=self.request.user).order_by('-pub_date')
...
class UserList(ListView):
	paginate_by = 25
	template_name = 'replica/dashboard/user_list.html'
...
class PageList(ListView):
	paginate_by = 25
	template_name = 'replica/dashboard/page_list.html'

	def get_queryset(self):
		if self.request.user.is_staff:
			return Entry.objects.pages().order_by('title')
		else:
			return Entry.objects.pages().filter(user=self.request.user).order_by('title')

	def get_context_data(self, **kwargs):
		context = super(PageList, self).get_context_data(**kwargs)
		context.update({'pub_date': True, 'heading': 'Pages', 'slug':'pages'})
		return context
